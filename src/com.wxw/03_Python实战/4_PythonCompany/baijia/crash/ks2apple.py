...
def get_crash_thread(report):
    ...
    return crash.get('currentThread', None)
...
def parse_system_info(report):
    ...
    headers.append('    OS Version:  {0} {1} ({2})'.format(_s('systemName'), _s('systemVersion'), _s('osVersion')))
    ...
    fmt = lambda x: "    Mem {0:6}:  {1:4} M".format(x, int(sys[x]) / 1024 / 1024)
    headers.append(fmt("memoryUsed"))
    headers.append(show_color(fmt("memoryFree")))
    headers.append(fmt("memorySize"))
...
def parse_error_info(report):
    ...
    if not error:
        print
        "waring: no error found in crash"
        return []
    mach = error['mach']
    signal = error['signal']
...
def parse_crash_reason(report):
    ...
    if not error:
        print
        "warning: no error found in crash"
        return []
    ...
    if ns_exception:
        result.append(fmt(ns_exception['name'], error.get('crashMessage', '')))
    elif zombie_exception(report):
        last_exception = get_last_exception(report)
        if (last_exception):
            result.append(fmt(last_exception['name'], last_exception['crashMessage']))
    elif user_exception:
        result.append(fmt(user_exception['name'], error['crashMessage']))
        ...
        if cpp_exception and cpp_exception.get('name'):
            result.append(fmt(cpp_exception['name'], error['crashMessage']))
...
def parse_backtrace(report, backtrace):
    ...
    for trace in backtrace.get('contents', []):
        ...
        if not img:
            print
            "error, no img found for pc: %s" % pc
...
def parse_binary_images(report):
    ...
    exe_path = system['executablePath']
    ...
    for image in images:
        try:
            ...
            if False:  # image_count <=10 and not is_image_wechat(name) and simple_symbol_find.uuid_find(uuid) == False:
                ...
                if name == 'WeChat':
                    print
                    '{0:>#18x} - {1:>#18x} {2}{3}  <{4}> {5}'.format(addr, addr + size - 1, is_base, name, uuid, path)
...
def parse_mem_info(report):
    ...
    fmt = lambda x: "    {0:6}: {1:4} M,".format(x, int(system[x]) / 1024 / 1024)
    result.append(fmt("memoryUsed"))
    result.append(fmt("memoryFree"))
    result.append(fmt("memorySize")[:-1])
    result.append('}')
...
def parse_app_info(report):
    ...
    try:
        if app_stats.get('app_launch_time'):
            app_stats['app_launch_time'] = datetime.fromtimestamp(app_stats.get('app_launch_time')).strftime(
                "%Y-%m-%d %H:%M:%S")
        app_stats['app_crash__time'] = get_time(report)

        app_stats['Process'] = '{0} [{1}]'.format(_s('processName'), _s('processId'))
...
def parse_extra_info(report):
    ...
    if last_exception:
        ...
        reason = last_exception['crashMessage']

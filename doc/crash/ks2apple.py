...
def parse_error_info(report):
    ...
    result = ['', 'Exception Type:  {0} ({1})'.format(exc_name, sig_name),
              'Exception Codes: {0} at {1:016x}'.format(code_name, int(addr)),
              'Crashed Thread:  {0}'.format(crash_thread)]
    ...
    for image in images:
        try:
            ...
            if False:  # image_count <=10 and not is_image_wechat(name) and simple_symbol_find.uuid_find(uuid) == False:
                ...
                if name == 'WeChat':
                    print '{0:>#18x} - {1:>#18x} {2}{3}  <{4}> {5}'.format(addr, addr + size - 1, is_base, name, uuid,
                                                                           path)
